...
class DBEngine:
    ...
    def execute(self, table_id, select_index, aggregation_index, conditions, lower=True):
        ...
        for tup in schema_str.split(', '):
            c, t = tup.split()
            schema[c] = t
        select1 = 'col{}'.format(select_index1)
        select2 = 'col{}'.format(select_index2)
        agg1 = agg_ops[aggregation_index1]
        agg2 = agg_ops[aggregation_index2]
        if agg1:
            select1 = '{}({})'.format(agg1, select1)
        if agg2:
            select2 = '{}({})'.format(agg2, select2)
        where_clause = []
        where_map = {}
        ...
        query = 'SELECT {} AS result1,{} AS result2 FROM {} {}'.format(select1,select2, table_id, where_str)
        #print query
