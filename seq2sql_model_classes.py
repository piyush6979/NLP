    ...
    def beam_forward(self, wemb_n, l_n, wemb_hpu, l_hpu, l_hs, tb,
        ...
        for b, idxs1 in enumerate(idxs):
            ...
            for i_wn, idxs11 in enumerate(idxs1):
                ...
                prob_conds11 = prob_w[b, idxs11[0], idxs11[1], idxs11[2],idxs11[3], idxs11[4], idxs11[5],idxs11[6], idxs11[7]]
...
class SCP(nn.Module):
    def __init__(self, iS=300, hS=100, lS=4, dr=0.3):
...
class SAP(nn.Module):
    def __init__(self, iS=300, hS=100, lS=4, dr=0.3, n_agg_ops=-1, old=False):
...
class WNP(nn.Module):
    def __init__(self, iS=300, hS=100, lS=4, dr=0.3, ):
        ...
        self.mL_w = 8  # max where condition number
        self.question_knowledge_dim = 5
        self.header_knowledge_dim = 3
        self.enc_h = nn.LSTM(input_size=iS, hidden_size=int(hS / 2),
                             num_layers=lS, batch_first=True,
...
class WOP(nn.Module):
    def __init__(self, iS=300, hS=100, lS=4, dr=0.3, n_cond_ops=3):
...
class WVP_se(nn.Module):
    ...
    def __init__(self, iS=300, hS=100, lS=4, dr=0.3, n_cond_ops=4, old=False):
        ...
        if old:
            ...
            self.wv_out = nn.Sequential(
                nn.Linear(8 * hS + self.question_knowledge_dim, hS),
                nn.Tanh(),
                nn.Linear(hS, 2)
            )
        # self.wv_out = nn.Sequential(
